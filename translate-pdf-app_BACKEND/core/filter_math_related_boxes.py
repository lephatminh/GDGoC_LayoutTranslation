...
def is_inside_math_box(pymupdf_box: Dict[str, float], math_box: Dict[str, float]) -> bool:
    ...
    x2_2 = math_box["x"] + math_box["width"] + MARGINAL_ERROR_HORIZONTAL
    y2_2 = y2_m = math_box["y"] + math_box["height"] + MARGINAL_ERROR_VERTICAL
    ...
    for text_box in text_boxes:
        ...
        for math_box in math_boxes:
            if is_inside_math_box(text_box, math_box):
                ...
                # logging.debug(f"Overlap: text={text_box}  math={math_box}")
                break
